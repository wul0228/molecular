...
(pubchem_load,pubchem_raw,pubchem_store,pubchem_db) = buildSubDir('pubchem')
...
def  downloadData(redownload = False,rawdir=None):
    ...
    if not os.path.exists(pjoin(pubchem_load,'pubchem.log')):

        initLogFile('pubchem',model_name,pubchem_load,rawdir=rawdir)
...
def extractPartData(rawdir,storedir):
    ...
    f = open(pjoin(pubchem_load,'errorStandardFile'),'w')
...
def insertData(storedir):
    '''
    this function is set to inser extracted data to mongdb database ,if data insert failed ,log to errorInsert.json in pubchem_load
    pares:
    ...
    with open(pjoin(pubchem_load, 'errorInsert_{}.json'.format(now)),'w')  as wf:
...
def updateData():
    ...
    pubchem_log =json.load(open(pjoin(pubchem_load,'pubchem.log')))
