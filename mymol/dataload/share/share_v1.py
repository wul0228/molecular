...
for _dir in datadirs:
    ...
    createDir(pjoin(mymol_path,_dir))

def buildSubDir(name):
    '''
    this function is to build all sub-directory for specified
    '''
    _load = os.path.join(dataload,name)

    _raw = os.path.join(dataraw,name)

    _store =  os.path.join(datastore,name)

    _db = pjoin(database,name)

    for _dir in [_raw,_store,_db]:

        createDir(_dir)      

    return (_load,_raw,_store,_db)
