...
def parseDiv(div):
    ...
    '''
    atext_href = dict([ (a.text.replace(' ','_').replace('.','_'), a['href']) for a in div.select('a') if not a.text.count('show all')])
...
def parseMolHtml(htmlpath,storedir):
    ...
    for tr in trs:
        ...
        if th == 'Entry':
            ...
            val = dict( [(a.text.replace(' ','_').replace('.','_'),a['href'])  for a in td.select('div > a')  if not a.text.count('show all')] )
...
def downloadData(redownload = False):
    ...
    if redownload or not existKeggFile or  choice == 'y':
        ...
        # 4. initialiaze log file
    if not os.path.exists(pjoin(kegg_load,'kegg_compound.log')):

        initLogFile('kegg_compound',model_name,kegg_load,mt=release)
...
def extractData(rawdir):
    ...
    rawdir -- the save directory of compound html
    '''
    storedir = pjoin(kegg_store,rawdir.split('dataraw/kegg/',1)[1].strip())
...
def insertData(storedir):
    ...
    collection_name = storedir.split('datastore/kegg/',1)[1].strip().split('/')[0].strip()
...
def updateData():
    ...
    '''
    kegg_compound_log =  json.load(open(pjoin(kegg_load,'kegg_compound.log')))
    ...
    if release != latest_edition:
        ...
        with open(pjoin(kegg_load,'drugbank.log'),'w') as wf:
...
if __name__ == '__main__':
    # main()
    # kegg_compound_raw = '/home/user/project/molecular/mymol/dataraw/kegg/kegg_compound_84.0+_171113104417'
    # storedir =  extractData( kegg_compound_raw)
    storedir = '/home/user/project/molecular/mymol/datastore/kegg/kegg_compound_84.0+_171113104417/'
    insertData(storedir)
