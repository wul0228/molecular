...
(chebi_load,chebi_raw,chebi_store,chebi_db) = buildSubDir('chebi')
...
def downloadData( redownload = False ):
    ...
    if redownload or not existChEBIFile or  choice == 'y':
        ...
        if not os.path.exists(pjoin(chebi_load,'chebi.log')):

            initLogFile('chebi',model_name,chebi_load,mt)
...
def updateData():
    ...
    chebi_log = json.load(open(pjoin(chebi_load,'chebi.log')))
    ...
    if latest_edition != last_record[0]:
        ...
        with open(pjoin(chebi_load,'chebi.log'),'w') as wf:
...
if __name__ =='__main__':
    ...
    main()
    # extractData(filepath = None, latest = False)
